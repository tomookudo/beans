...
// Beans
async function getEventsByMail(){
  const mailaddr = document.getElementById('ev-mail').value;
  console.log(mailaddr);
  get_mailaddr = mailaddr;
  let userHttp = `users/`+mailaddr+`/calendarView`;
  ...
  console.log(`users --- Converted: ${ianaTimeZone}`);
  ...
  let endOfWeek = moment(startOfWeek).add(100, 'day');
  ...
  try {
    ...
    let response = await graphClient
      // .api('/me/calendarview')
      // .api(`users/kou--227@outlook.com/calendarView`)
      .api(userHttp)
...
async function getEvents() {
  ...
  // Moment needs IANA format
  let ianaTimeZone = getIanaFromWindows(user.mailboxSettings.timeZone);
  console.log(`me --- Converted: ${ianaTimeZone}`);
